...
import { MockERC20__factory, ProtocolSettings__factory, Protocol__factory, Vault__factory, ERC4626BribeVault__factory } from "../typechain";
...
async function main() {
  ...
  const protocolAddress = await deployContract("Protocol", []);
  const protocol = Protocol__factory.connect(protocolAddress, deployer);
  ...
  const deployVault = async (asset: string, pool: string, name: string, index: number = 1) => {
    const vaultAddress = await deployContract(
      "Vault",
      [protocolAddress, protocolSettingsAddress, redeemPoolFactoryAddress, bribesPoolFactoryAddress, pool, asset, `Wand p${name}`, `p${name.slice(0, 10)}`],
      `${name}_${index}_Vault`,
      {
        libraries: {
  ...
  await deployYeetVault("0x0001513F4a1f86da0f02e647609E9E2c630B3a14", "0x208008F377Ad00ac07A646A1c3eA6b70eB9Fc511", "Wand pBERAYEET", "pBERAYEET");
}
